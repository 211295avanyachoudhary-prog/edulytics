...
import { User, PenLine, BookOpen, Shield, Star, Calendar, ArrowRight } from 'lucide-react'
...
export default function ProfilePage() {
  ...
  const trustColor = profile.trust_score >= 2.5 ? 'from-amber-500 to-orange-500' : profile.trust_score >= 1.5 ? 'from-brand-500 to-violet-500' : 'from-slate-500 to-slate-600'
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-6">
              ...
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  <span className={`px-2.5 py-0.5 rounded-full bg-gradient-to-r ${trustColor} text-white text-xs font-semibold`}>
                    {trustLevel}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-1.5 mt-2">
                  ...
                  <span className="text-white/40 text-xs">Joined {formatDate(profile.created_at)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              },
              {
                icon: Shield,
                label: 'Trust Score',
                value: profile.trust_score?.toFixed(2),
              ...
              <div key={label} className="bg-white border border-slate-100 rounded-2xl p-5 shadow-card text-center">
                <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center mx-auto mb-3`}>
          ...
          <div className="bg-gradient-to-br from-brand-50 to-violet-50 border border-brand-100 rounded-2xl p-5">
            ...
            <div className="flex items-center gap-3 mb-3">
              <div className="flex-1 h-3 bg-white rounded-full overflow-hidden shadow-inner">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{ width: `${(profile.trust_score / 5) * 100}%` }}
            ...
            <p className="text-xs text-slate-600">
              Write detailed, consistent reviews to increase your trust score. Higher trust means your ratings carry more weight in the algorithm.
            </p>
          </div>
            ...
            {reviews.length === 0 ? (
              <div className="bg-white border-2 border-dashed border-slate-200 rounded-3xl p-10 text-center">
                ...
                <Link href="/school/select">
                  <button className="inline-flex items-center gap-2 px-5 py-2.5 bg-brand-600 text-white font-semibold rounded-xl text-sm hover:bg-brand-700 transition-colors">
                    Find a School <ArrowRight className="w-4 h-4" />
                  </button>
                </Link>
              </div>
            ) : (
              <div className="space-y-4">
                {reviews.map((review, i) => (
                  <Link key={review.review_id} href={`/school/${review.school_id}`}>
