...
function AuthForm() {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-blue-50/30 p-4">
        ...
        <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}
          ...
          <div className="px-8 pt-8 pb-6 border-b border-slate-100">
            <div className="flex items-center gap-2.5 mb-6">
              ...
              <span className="font-bold text-lg text-slate-900">edu<span className="text-indigo-600">lytics</span></span>
            </div>
            <div className="flex bg-slate-100 rounded-2xl p-1">
              {['Sign In','Sign Up'].map((t,i) => (
                <button key={t} onClick={() => { setIsSignup(i===1); setError('') }}
                  className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all ${isSignup===(i===1)?'bg-white text-slate-900 shadow-sm':'text-slate-500 hover:text-slate-700'}`}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <div className="px-8 py-6">
            <AnimatePresence mode="wait">
              <motion.form key={isSignup?'up':'in'} initial={{ opacity:0, x:isSignup?20:-20 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0 }}
                ...
                {isSignup && (
                  <div>
                    ...
                    <input type="text" required value={form.username} onChange={set('username')} placeholder="yourname123" minLength={3}
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition-all" />
                  </div>
                )}
                <div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type={showPwd?'text':'password'} required value={form.password} onChange={set('password')}
                      ...
                      className="w-full px-4 py-3 pr-11 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition-all" />
